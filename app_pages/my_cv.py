...
def render_or_update_model_info(model_name):
    ...
    Returns:
        None
    """
    with open("./design/mi_csv/styles.css") as f:
        css = f.read()
    st.markdown('<style>{}</style>'.format(css), unsafe_allow_html=True)

    with open("./design/mi_csv/content.html") as f:
...
def reset_chat_history():
    ...
    """
    if "messages_mi_cv" in st.session_state:
        st.session_state.messages_mi_cv = []
...
if "messages_mi_cv" not in st.session_state:
    st.session_state.messages_mi_cv = []
...
for message in st.session_state.messages_mi_cv:
...
if st.session_state.prompt:
    ...
    with st.chat_message("assistant"):
        ...
        response = lu.invoke_chain(
            ...
            messages=st.session_state.messages_mi_cv,
    ...
    st.session_state.messages_mi_cv.append({"role": "user", "content": st.session_state.prompt, "aux": {}})
    st.session_state.messages_mi_cv.append({"role": "assistant", "content": lu.invoke_chain.response, "aux": lu.invoke_chain.aux})
